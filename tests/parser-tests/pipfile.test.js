...
test('Should return all deps from package file', async () => {
    const { parser, packageFileContent } = await setupTest('python/pipfile');
    ...
    assert.equal(store.store['python:pipfile'], {
...
test('Should return latest & all versions for all deps from package file', async () => {
    ...
    for (const dep of depList) {
        const stored = store.get('python:pipfile', dep);
        assert.ok(stored);
...
test('Should return current versions from package file', async () => {
    const { parser, packageFilePath, packageFileContent } = await setupTest('python/pipfile');
    ...
    assert.equal(store.store['python:pipfile'], {
        ...
        'pylint':            { semverVersion: '>=2.1.1', currentVersion: '==2.3.1' },
    });
});
